...
import { findAllParent, findMenuItem, getMenuItemFromURL } from '@/admin/helpers/menu';
...
const MenuItemLink = ({ item, className }) => {
  // Tự động thêm ?module=admin vào URL
  const urlWithModule = `${item.url ?? ''}${item.url?.includes('?') ? '&' : '?'}module=admin`;

  return (
    <Link to={urlWithModule} target={item.target} className={clsx(className, { disabled: item.isDisabled })}>
      {item.icon && <span className="nav-icon"><IconifyIcon icon={item.icon} /></span>}
      <span className="nav-text">{item.label}</span>
...
const AppMenu = ({ menuItems }) => {
  ...
  const activeMenu = useCallback(() => {
    const trimmedURL = pathname.split('?')[0]; // bỏ query để so sánh
